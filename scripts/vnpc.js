  ...
  static async showVNPC(actor){
    ...
    try {
       ...
       if (existing_vnpc != undefined){
            ...
            try {
                VNPC.hideVNPC(dismissable_actor)
				console.log("calling hideVNPC on dismissable_actor")

            }
            catch (err){
                console.log("error with hideNPC call, trying macro fallback")
                VNPC.hideVNPC({actor: dismissable_actor})
            }
	...
	options.push(
		{
			...
			callback: (li) => {
				const actor = game.actors?.get(li.data("documentId"));
				console.log("VNPC | menu item called")
				console.log(actor)
				VNPC.showVNPC(actor)
